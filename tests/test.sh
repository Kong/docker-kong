...
function tmessage {
  echo -e "$T_COLOR_YELLOW$T_PROJECT_NAME: $*$T_COLOR_CLEAR"
}
...
function tfinish {
  ...
  if [[ $T_INIT_COUNT -eq 0 ]]; then
    ...
    if [ "$T_COUNT_FAILURE" -eq 0 ] && [ "$T_COUNT_SUCCESS" -gt 0 ]; then
      if [ $T_FIGLET_AVAILABLE -eq 0 ]; then
        echo -e -n "$T_COLOR_GREEN"
        figlet -c -w 120 "Success!"
        echo -e -n "$T_COLOR_CLEAR"
      else
      ...
      if [ $T_FIGLET_AVAILABLE -eq 0 ]; then
        echo -e -n "$T_COLOR_RED"
        figlet -c -w 120 "Failed!"
        echo -e -n "$T_COLOR_CLEAR"
      else
